...
def db_conn(monkeypatch: pytest.MonkeyPatch) -> sqlite3.Connection:
    ...
    # tools.py, importer.py, exporter.py, vitality.py, mempalace_import.py, and
    # dbs_import.py use `from remind_me_mcp.db import _get_db` which creates
    # separate bindings — patch those local references directly so tool
    ...
    monkeypatch.setattr(_dbs_mod, "_get_db", lambda: db)
...
def db_conn_concurrent(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> sqlite3.Connection:
    ...
    *whole* database at a time — far more contention-prone than production
    ...
    for mod in (
        _db_mod, _api_mod, _tools_mod, _importer_mod, _exporter_mod,
        _vitality_mod, _mempalace_mod, _dbs_mod,
    ):
...
def memory_factory(db_conn: sqlite3.Connection):
    ...
    def create_memory(**overrides) -> dict:
        ...
        v5_cols = {
            k: defaults[k]
            for k in ("status", "vitality", "memory_type", "decay_rate", "base_weight", "access_count", "accessed_at", "source_capture_id", "capture_id", "subject", "predicate", "object", "superseded_by", "doc_id", "chunk_index", "deleted_at", "remind_at", "sensitive")
            if k in defaults
        }
        if v5_cols:
