...
| FT-06 | med | todo | Govern — export the entity graph: include `entities` and `memory_entities` in `export_memories` / `GET /api/export` (with import-side restore for round-trip), so backups capture the full knowledge graph, not just memories | — |
