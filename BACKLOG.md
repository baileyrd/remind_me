...
| SY-14 | med | todo | `remind_me_sync_reconcile`: diff a node against the hub's `/stats` (SY-13) and return per-category drift with a verdict — `in-sync` / `pull-lag` (hub ahead in live categories, benign) / `node-ahead` (push isn't landing) / `fault` (drift in bulk categories). The encoded judgment is the deliverable; raw counts already exist but the benign-vs-fault distinction is a sign change that's easy to miss by eye | [#90](https://github.com/baileyrd/remind_me/issues/90) |
